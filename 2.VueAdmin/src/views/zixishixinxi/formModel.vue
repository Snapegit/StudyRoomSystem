		...
		<el-dialog v-model="formVisible" :title="formTitle" width="80%" destroy-on-close :fullscreen='false'>
			<el-form class="formModel_form" ref="formRef" :model="form" label-width="$template2.back.add.form.base.labelWidth" :rules="rules">
				<el-row>
					...
					<el-col :span="24">
						<el-form-item prop="zixishitupian"
									  label="自习室图片"
						>
							<uploads
					...
					<el-col :span="24">
						<el-form-item label="自习室介绍" prop="zixishijieshao">
							<editor :value="form.zixishijieshao" placeholder="请输入自习室介绍" :readonly="!isAdd||disabledForm.zixishijieshao?true:false"
								class="list_editor" @change="(e)=>editorChange(e,'zixishijieshao')"></editor>
						</el-form-item>
					</el-col>
				</el-row>
			</el-form>
...
<script setup>
	...
	const getInfo = ()=>{
		context?.$http({
			...
			res.data.data.zixishijieshao = res.data.data.zixishijieshao?(res.data.data.zixishijieshao.replace(reg,'../../../cl78696306/file')):'';
	...
	const init=(formId=null,formType='add',formNames='',row=null,table=null,statusColumnName=null,tips=null,statusColumnValue=null)=>{
		...
		if(formType == 'add'){
			...
			for(let x in row){
				...
				if(x=='zixishijieshao'){
					form.value.zixishijieshao = row[x];
					disabledForm.value.zixishijieshao = true;
					continue;
				}
				if(x=='zixishizuowei'){
		...
		zixishileixingLists.value = "考研自习室,四六级自习室,考公考编自习室,自由自习室".split(',')
	}
	//初始化
	//声明父级调用
	defineExpose({
